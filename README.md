...
5. **表格出现 minipage 相关报错**：通常是因为表格过宽，页面无法完整容纳，Pandoc 会尝试将单元格内容包装为 minipage 并进行缩放。可通过以下方式处理：减少表格列数、精简列内文字、将大表拆分为续表，或改为转置表格。

6. **浮动体位置问题**：图片浮动体默认参数为 `!htbp`，表示按 "here, top, bottom, page" 的顺序尝试放置图片。
