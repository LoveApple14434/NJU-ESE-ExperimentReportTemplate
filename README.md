...
1. **文档类**：模板默认使用 `ctexart` 文档类，支持中文排版和章节标题格式。

1. **格式调整**：如需调整页边距、行距等格式，可修改 `geometry` 和 `linespread` 参数。

1. **配置**：模板已配置超链接和页眉样式，无需额外设置。

1. ~~**表格出现 minipage 相关报错**：通常是因为表格过宽，页面无法完整容纳，Pandoc 会尝试将单元格内容包装为 minipage 并进行缩放。可通过以下方式处理：减少表格列数、精简列内文字、将大表拆分为续表，或改为转置表格。~~

   此问题已修复，其实是少`calc`和`array`这两个宏包导致的（）

1. **浮动体位置问题**：图片浮动体默认参数为 `!htbp`，表示按 "here, top, bottom, page" 的顺序尝试放置图片。

## 🛠 更新与贡献

如果实验中心更新了格式要求，或者您对本模板有任何改进建议，欢迎提交 Issue；我会尽快处理，也欢迎直接一步到位提交 Pull Request ！
